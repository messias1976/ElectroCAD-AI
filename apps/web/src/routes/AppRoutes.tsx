...
class RouteErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> { state = { error: null as Error | null }; static getDerivedStateFromError(error: Error) { return { error }; } render() { if (this.state.error) return <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-6 text-red-900"><h2 className="text-xl font-bold">Não foi possível abrir esta página</h2><p className="mt-2 text-sm">{this.state.error.message}</p><button onClick={() => window.location.reload()} className="mt-4 rounded-lg bg-red-700 px-4 py-2 font-semibold text-white">Recarregar</button></div>; return this.props.children; } }
function LoadingPage() { return <div className="flex min-h-screen items-center justify-center bg-slate-950 text-white"><div className="rounded-2xl border border-white/10 bg-white/5 px-6 py-5 text-center shadow-xl"><div className="mb-2 text-2xl">⚡ ElectroCAD AI</div><div className="text-sm text-slate-300">Carregando...</div></div></div>; }
export default function AppRoutes() { return <RouteErrorBoundary><Suspense fallback={<LoadingPage />}><Routes>
