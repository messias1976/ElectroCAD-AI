...
export default function PlantDesignerPage() {
  ...
  function roomDown(e:PointerEvent<SVGGElement>,id:number){if(tool!=='select')return;e.stopPropagation();e.currentTarget.setPointerCapture(e.pointerId);setDragRoom(id);}
  ...
  return <div className="min-h-full space-y-5 pb-8 text-left">
    ...
    <div className="grid gap-5 rounded-2xl border bg-white p-3 shadow-sm lg:grid-cols-[1fr_260px]">
      <div className="overflow-auto rounded-xl bg-slate-100 p-2"><svg viewBox="0 0 740 500" className="min-w-[740px] rounded-xl bg-white shadow-inner" preserveAspectRatio="xMidYMid meet" onClick={canvasClick} onPointerMove={move} onPointerUp={finishDrag} onPointerCancel={finishDrag} onPointerLeave={e=>{if(dragPoint!==null||dragRoom!==null||resizeRoom!==null||dragOpening||resizeOpening)move(e as unknown as PointerEvent<SVGSVGElement>);}} style={{touchAction:'none'}}>
        ...
        {points.map(p=>{const c=p.kind==='Luz'?'#facc15':p.kind==='TUG'?'#60a5fa':'#fb923c';const room=rooms.find(r=>r.id===p.roomId);const hover=hovered===p.id;return <g key={p.id} onPointerDown={e=>{e.stopPropagation();if(tool==='select'){e.currentTarget.setPointerCapture(e.pointerId);setSelected(p.id);setDragPoint(p.id);setMessage(`Movendo ${p.label}. Solte no local desejado.`);}}} onPointerUp={e=>{if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);}} onMouseEnter={()=>setHovered(p.id)} onMouseLeave={()=>setHovered(null)} className="cursor-grab touch-none"><circle cx={p.x} cy={p.y} r={selected===p.id?16:13} fill={c} stroke="#0f172a" strokeWidth="2"/><circle cx={p.x} cy={p.y} r="24" fill="transparent" pointerEvents="all"/><text x={p.x} y={p.y+4} textAnchor="middle" fontSize="8" fontWeight="800" pointerEvents="none">{p.label}</text>{hover&&<g pointerEvents="none"><rect x={Math.min(570,p.x+15)} y={Math.max(5,p.y-70)} width="160" height="58" rx="8" fill="#0f172a"/><text x={Math.min(580,p.x+25)} y={Math.max(22,p.y-53)} fontSize="10" fontWeight="700" fill="white">{p.label} · {p.kind}</text><text x={Math.min(580,p.x+25)} y={Math.max(38,p.y-37)} fontSize="9" fill="#e2e8f0">{room?.name} · {p.watts} W · {p.voltage} V</text><text x={Math.min(580,p.x+25)} y={Math.max(52,p.y-23)} fontSize="8" fill="#cbd5e1">Arraste para reposicionar</text></g>}</g>})}
      </svg></div>
