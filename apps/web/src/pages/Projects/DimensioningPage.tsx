...
import { ArrowLeft, Calculator, FileText, Printer, Save, ShieldCheck, Wrench } from 'lucide-react';
...
function money(v: number) { return `R$ ${Number(v || 0).toFixed(2).replace('.', ',')}`; }
...
export default function DimensioningPage() {
  ...
  const [result, setResult] = useState<any>(null); const [budget, setBudget] = useState<any>(null); const [message, setMessage] = useState('Selecione o projeto correto para iniciar o dimensionamento e o orçamento.'); const [saving, setSaving] = useState(false);
  ...
  async function saveResult() {
    if (!project || !result || !budget) { setMessage('Calcule o projeto antes de salvar.'); return; }
    setSaving(true);
    try {
      const currentProjectData = parse(project.projectData);
      const savedData = { ...currentProjectData, dimensioning: { savedAt: new Date().toISOString(), settings: { voltageSystem, phases, grounding, mainBreaker: asNumber(mainBreaker), pf: asNumber(pf), dropLimit: asNumber(dropLimit), ambient: asNumber(ambient), rccbRequired, rccbSensitivity: asNumber(rccbSensitivity), groupingFactor: asNumber(groupingFactor), temperatureFactor: asNumber(temperatureFactor) }, result, budget } };
      await apiFetch(`/projects/${encodeURIComponent(project.id)}`, { method: 'PUT', body: JSON.stringify({ projectData: JSON.stringify(savedData) }) });
      setMessage(`Dimensionamento e orçamento salvos no projeto “${project.name}”.`);
    } catch (e) { setMessage(`Não foi possível salvar: ${e instanceof Error ? e.message : 'erro desconhecido'}`); }
    finally { setSaving(false); }
  }

  function printResult() {
    if (!project || !result || !budget) { setMessage('Calcule o projeto antes de imprimir.'); return; }
    window.print();
  }
  ...
  return <div className="dimensioning-page mx-auto max-w-6xl space-y-5 pb-12">
    ...
    <div className="no-print-dimensioning flex flex-wrap items-center justify-between gap-3"><div><Link to={projectId ? `/projetista?projectId=${encodeURIComponent(projectId)}` : '/projetista'} className="text-sm text-slate-500"><ArrowLeft size={15} className="mr-1 inline"/>Voltar ao projetista</Link><h2 className="mt-1 text-3xl font-bold text-slate-900">Dimensionamento e orçamento</h2><p className="mt-1 text-sm text-slate-500">Motor preliminar baseado nas condições informadas, com materiais por quantidade/especificação e orçamento somente de mão de obra.</p></div><div className="flex flex-wrap gap-2"><button onClick={run} disabled={!project || !circuits.length} className="rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Calculator size={17} className="mr-2 inline"/>Calcular projeto</button><button onClick={saveResult} disabled={!result || saving} className="rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 font-semibold text-emerald-800 disabled:opacity-50"><Save size={17} className="mr-2 inline"/>{saving ? 'Salvando...' : 'Salvar'}</button><button onClick={printResult} disabled={!result} className="rounded-xl border border-slate-300 bg-white px-4 py-3 font-semibold text-slate-700 disabled:opacity-50"><Printer size={17} className="mr-2 inline"/>Imprimir / PDF</button></div></div>

    <section className="no-print-dimensioning rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-sm"><div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end"><label className="text-sm font-bold text-slate-800">Projeto para dimensionar e orçar<select value={projectId} onChange={e => handleProjectChange(e.target.value)} className="mt-2 w-full rounded-xl border border-blue-300 bg-white px-4 py-3 text-base font-semibold text-slate-900 outline-none focus:border-blue-600"><option value="">Selecione um projeto...</option>{projects.map(p => <option key={p.id} value={p.id}>{p.name}{p.client?.name ? ` — ${p.client.name}` : ''}</option>)}</select></label>{project ? <div className="rounded-xl bg-white px-4 py-3 text-sm"><div className="font-semibold text-slate-900">Projeto ativo</div><div className="text-slate-600">{project.name}{project.client?.name ? ` · Cliente: ${project.client.name}` : ''}</div></div> : <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Selecione um projeto antes de calcular.</div>}</div></section>

    <div className="no-print-dimensioning rounded-xl border bg-slate-50 px-4 py-3 text-sm text-slate-600">{message}</div>

    <section className="dimensioning-section rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-lg font-bold">Dados do pré-dimensionamento</h3><div className="mt-4 grid gap-4 md:grid-cols-4">
    ...
    {result && <>
      <section className="dimensioning-section grid gap-4 md:grid-cols-4"><div className="dimensioning-card rounded-2xl border bg-white p-5"><div className="text-sm text-slate-500">Potência total</div><div className="mt-1 text-2xl font-bold">{result.totalPower_kW} kW</div></div><div className="dimensioning-card rounded-2xl border bg-white p-5"><div className="text-sm text-slate-500">Corrente estimada</div><div className="mt-1 text-2xl font-bold">{result.totalCurrent_A} A</div></div><div className="dimensioning-card rounded-2xl border bg-white p-5"><div className="text-sm text-slate-500">Circuitos</div><div className="mt-1 text-2xl font-bold">{result.circuits.length}</div></div><div className="dimensioning-card rounded-2xl border bg-white p-5"><div className="text-sm text-slate-500">Mão de obra sugerida</div><div className="mt-1 text-2xl font-bold">{money(budget?.suggested || 0)}</div></div></section>
      <section className="dimensioning-section rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-xl font-bold"><ShieldCheck size={19} className="mr-2 inline"/>Diagnóstico</h3><p className="mt-2">{result.diagnosis}</p><div className="mt-4 space-y-4">{result.circuits.map((c: any) => <div key={c.id} className="dimensioning-card rounded-xl border p-4"><div className="flex flex-wrap justify-between gap-2"><b>{c.id} — {c.name}</b><span>{c.ok ? '✅ OK' : '⚠️ Revisar'}</span></div><div className="mt-2 grid gap-2 text-sm md:grid-cols-6"><span>P: <b>{c.totalPower_W} W</b></span><span>Ib: <b>{c.ib_A} A</b></span><span>Ib projeto: <b>{c.designCurrent_A} A</b></span><span>Disj.: <b>{c.breaker_A ?? '—'} A</b></span><span>Condutor: <b>{c.conductor_mm2 ?? '—'} mm²</b></span><span>Iz: <b>{c.iz_A ?? '—'} A</b></span></div><div className="mt-2 text-sm">ΔV: <b>{c.voltageDrop_V ?? '—'} V ({c.voltageDropPercent ?? '—'}%)</b> · Curva: <b>{c.breakerCurve}</b>{c.motor && <> · Partida: <b>{c.startingCurrent_A} A / {c.startingVoltageDropPercent}%</b></>}</div><div className="mt-3 text-sm"><b>Cálculos/Justificativas:</b> {c.motor ? `motor ${c.motor.power_kW} kW, ${c.motor.phases} fase(s), η=${c.motor.efficiency}, fp=${c.motor.powerFactor}; Ib = ${c.ib_A} A; Ib projeto = ${c.designCurrent_A} A.` : `P = ${c.totalPower_W} W; Ib = P/(V×fp) = ${c.ib_A} A.`} Queda de tensão = {c.voltageDrop_V ?? '—'} V ({c.voltageDropPercent ?? '—'}%).</div>{c.alerts.length > 0 && <div className="mt-3 text-sm text-amber-800"><b>⚠️ Alertas:</b><ul className="ml-5 list-disc">{c.alerts.map((a: string) => <li key={a}>{a}</li>)}</ul></div>}</div>)}</div></section>
      <section className="dimensioning-section rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-xl font-bold"><Wrench size={19} className="mr-2 inline"/>Lista de materiais — sem preços</h3><p className="mt-1 text-sm text-slate-500">O sistema calcula quantidades e especificações. O preço deve ser consultado na loja/fornecedor escolhido.</p><div className="mt-4 overflow-x-auto"><table className="print-small min-w-full text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="px-3 py-2">Categoria</th><th className="px-3 py-2">Item</th><th className="px-3 py-2">Especificação</th><th className="px-3 py-2">Qtd.</th><th className="px-3 py-2">Un.</th><th className="px-3 py-2">Base</th></tr></thead><tbody>{result.materialSchedule.map((m: any, i: number) => <tr key={`${m.item}-${i}`} className="border-b last:border-0"><td className="px-3 py-2">{m.category}</td><td className="px-3 py-2 font-semibold">{m.item}</td><td className="px-3 py-2">{m.specification}</td><td className="px-3 py-2">{m.quantity}</td><td className="px-3 py-2">{m.unit}</td><td className="px-3 py-2 text-slate-500">{m.basis}</td></tr>)}</tbody></table></div></section>
      <section className="dimensioning-section grid gap-4 md:grid-cols-2"><div className="dimensioning-card rounded-2xl border bg-white p-5"><h3 className="font-bold"><FileText size={18} className="mr-2 inline"/>Orçamento — somente mão de obra</h3><p className="mt-2 text-3xl font-bold">{money(budget.suggested)}</p><p className="text-sm text-slate-500">Faixa sugerida: {money(budget.rangeMin)} a {money(budget.rangeMax)}</p><ul className="mt-3 space-y-1 text-sm">{Object.entries(budget.breakdown).map(([k, v]) => <li key={k} className="flex justify-between"><span>{k}</span><b>{money(Number(v))}</b></li>)}</ul><p className="mt-4 text-xs text-slate-500">{budget.note}</p></div><div className="dimensioning-card rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="font-bold">⚠️ Próximos passos</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm">{result.alerts.map((a: string) => <li key={a}>{a}</li>)}</ul><p className="mt-4 text-xs text-slate-600">{result.disclaimer}</p></div></section>
    </>}
  </div>;
}
