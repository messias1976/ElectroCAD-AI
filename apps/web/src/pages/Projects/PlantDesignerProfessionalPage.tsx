...
import { ArrowLeft, BrainCircuit, FileText, Lightbulb, MousePointer2, Printer, Save, Trash2, WandSparkles, Square as WindowIcon, DoorOpen, Zap, Maximize2, Minus, Plus } from 'lucide-react';
...
function expandPoints(data: any, rooms: Room[]): Point[] {
  ...
  source.forEach((p: any, index: number) => {
    ...
    for (let i = 0; i < qty; i++) {
      ...
      const radius = Math.min(38, Math.max(18, Math.min(room?.w || 80, room?.h || 80) / 5));
...
export default function PlantDesignerProfessionalPage() {
  ...
  const bounds = useMemo(() => {
    const maxX = Math.max(900, ...rooms.map(r => r.x + r.w + 80));
    const maxY = Math.max(600, ...rooms.map(r => r.y + r.h + 80));
    return { width: maxX, height: maxY };
  }, [rooms]);
  const stats = useMemo(() => ({
    points: points.length,
    rooms: rooms.length,
    tug: points.filter(p => p.kind === 'TUG').length,
    tue: points.filter(p => p.kind === 'TUE').length,
    light: points.filter(p => p.kind === 'Luz').length,
    watts: points.reduce((sum, p) => sum + p.watts, 0),
  }), [points]);
  ...
  function canvasDrop(kind: Kind, e: ReactPointerEvent<SVGSVGElement>) { const p = svgPoint(e); addPoint(kind, p.x, p.y); }
  async function save() {
  ...
  return <div className="plant-professional min-h-full space-y-4 pb-8 text-left">
    ...
    <div className="plant-no-print grid gap-3 lg:grid-cols-[240px_1fr]">
      ...
      <div className="plant-canvas-wrap overflow-auto rounded-2xl border bg-slate-100 p-3"><svg className="plant-canvas min-w-[720px] rounded-xl border bg-white" viewBox={`0 0 ${bounds.width} ${bounds.height}`} width="100%" preserveAspectRatio="xMinYMin meet" style={{ touchAction: 'none', userSelect: 'none' }} onPointerMove={move} onPointerUp={finishDrag} onPointerCancel={finishDrag} onPointerLeave={finishDrag} onPointerDown={e => { if (tool === 'Luz' || tool === 'TUG' || tool === 'TUE') canvasDrop(tool, e); }}>
        ...
        {points.map(p => { const cfg = palette[p.kind]; const selected = selected === p.id; return <g key={p.id} onPointerDown={e => { e.stopPropagation(); if (tool !== 'select') return; e.currentTarget.setPointerCapture(e.pointerId); setSelected(p.id); setDrag({ type: 'point', id: p.id }); setMessage(`Movendo ${p.label}`); }} className="cursor-grab"><circle cx={p.x} cy={p.y} r={selected ? 17 : 14} fill={cfg.color} stroke="#0f172a" strokeWidth="2"/><circle cx={p.x} cy={p.y} r="26" fill="transparent" pointerEvents="all"/><text x={p.x} y={p.y + 4} textAnchor="middle" fontSize="8" fontWeight="800" pointerEvents="none">{p.label}</text></g>; })}
      </svg></div>
    </div>
    <div className="plant-no-print rounded-2xl border bg-white p-3 shadow-sm"><div className="grid grid-cols-3 gap-2 plant-tabs"><button onClick={() => setActiveTab('resumo')} className={`rounded-xl px-4 py-3 font-semibold ${activeTab === 'resumo' ? 'bg-slate-900 text-white' : 'hover:bg-slate-50'}`}><FileText size={16} className="mr-1 inline"/>Resumo</button><button onClick={() => setActiveTab('editor')} className={`rounded-xl px-4 py-3 font-semibold ${activeTab === 'editor' ? 'bg-blue-600 text-white' : 'hover:bg-slate-50'}`}><MousePointer2 size={16} className="mr-1 inline"/>Editor 2D</button><button onClick={() => setActiveTab('professor')} className={`rounded-xl px-4 py-3 font-semibold ${activeTab === 'professor' ? 'bg-violet-600 text-white' : 'hover:bg-slate-50'}`} onClick={() => { setActiveTab('professor'); if (projectId) window.location.href = `/professor?projectId=${encodeURIComponent(projectId)}`; }}><BrainCircuit size={16} className="mr-1 inline"/>Professor IA</button></div>
