...
export class SubscriptionsService {
  ...
  async createAsaasSubscription(userId: string, planName: string, billingType: AsaasBillingType = 'UNDEFINED', cpfCnpj = '', customerName = '', customerEmail = '') {
    ...
    const document = this.normalizeCpfCnpj(cpfCnpj); const name = customerName.trim(); const email = customerEmail.trim().toLowerCase();
    if (name.length < 3) throw new BadRequestException('Informe seu nome completo.');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new BadRequestException('Informe um e-mail válido.');
    let customerId = user.asaasCustomerId;
    if (customerId) {
      try {
        await this.asaas.updateCustomer(customerId, { name, email, cpfCnpj: document });
      } catch (error: any) {
        const description = String(error?.response?.data?.errors?.[0]?.description || error?.message || '').toLowerCase();
        if (!description.includes('cliente excluído') && !description.includes('cliente excluido')) throw error;
        this.logger.warn(`Cliente Asaas ${customerId} está excluído; criando um novo cliente.`);
        const customer = await this.asaas.createCustomer({ name, email, cpfCnpj: document, externalReference: user.id });
        customerId = customer?.id;
        if (!customerId) throw new BadRequestException('Asaas não retornou o ID do novo cliente.');
        await this.prisma.user.update({ where: { id: user.id }, data: { asaasCustomerId: customerId, username: name, email } });
      }
    } else {
      const customer = await this.asaas.createCustomer({ name, email, cpfCnpj: document, externalReference: user.id });
      customerId = customer?.id;
