...
export class SubscriptionsService {
  ...
  async createAsaasSubscription(userId: string, planName: string, billingType: AsaasBillingType = 'UNDEFINED', cpfCnpj = '', customerName = '', customerEmail = '') {
    ...
    if (customerId) await this.asaas.updateCustomer(customerId, { name, email, cpfCnpj: document });
    else { const customer = await this.asaas.createCustomer({ name, email, cpfCnpj: document, externalReference: user.id }); customerId = customer?.id; if (!customerId) throw new BadRequestException('Asaas não retornou o ID do cliente.'); await this.prisma.user.update({ where: { id: user.id }, data: { asaasCustomerId: customerId, username: name, email } }); }
  ...
  async changeStatus(userId: string, status: string) {
    ...
    if (!subscription) {
      return this.prisma.subscription.create({ data: { userId, plan: 'Manual', provider: 'MANUAL', providerId: '', status: normalizedStatus } });
    }
