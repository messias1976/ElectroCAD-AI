...
export function getAiConfig() {
  return {
    ...
    model: sessionStorage.getItem(AI_MODEL_STORAGE) ?? 'gpt-5-mini',
  };
}

export function saveAiConfig(apiKey: string, model: string) {
  if (apiKey.trim()) sessionStorage.setItem(AI_KEY_STORAGE, apiKey.trim());
...
export async function saveAiConfigServer(apiKey: string, model: string, enabled = true) {
  const result = await apiFetch('/ai/config', {
    method: 'PUT',
    body: JSON.stringify({ apiKey: apiKey.trim() || undefined, model: model.trim() || 'gpt-5-mini', enabled }),
  });
  saveAiConfig('', model);
  return result;
}
...
export async function testAiConnection(apiKey?: string, model?: string) {
  ...
  return apiFetch('/ai/test', {
    method: 'POST',
    body: JSON.stringify({
      apiKey: apiKey?.trim() || config.apiKey || undefined,
      model: model?.trim() || config.model,
    }),
  });
}

export async function clearAiConfigServer() {
  const result = await apiFetch('/ai/config', { method: 'DELETE' });
  clearAiConfig();
  return result;
}

export async function askProfessor(message: string, project?: unknown, projectId?: string) {
  return apiFetch('/ai/chat', {
    method: 'POST',
    body: JSON.stringify({ message, project, projectId }),
  });
}

export function getCurrentProjectContext() {
  const project = localStorage.getItem('electrocad-project');
  const plant = localStorage.getItem('electrocad-plant');
  return {
    project: project ? JSON.parse(project) : null,
    plant: plant ? JSON.parse(plant) : null,
  };
}
